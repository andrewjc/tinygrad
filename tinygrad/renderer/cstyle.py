...
class CStyleLanguage(Renderer):
  ...
  code_for_op: Dict = {
    ...
    BinaryOps.DIV: lambda a, b, dtype: f"({a}/{b})", BinaryOps.MAX: lambda a, b, dtype: f"max({a},{b})",
    BinaryOps.MOD: lambda a, b, dtype: f"({a}%{b})",
    BinaryOps.CMPLT: lambda a, b, dtype: f"({a}<{b})", BinaryOps.CMPEQ: lambda a, b, dtype: f"({a}=={b})",
  ...
  def render_const(self, x: ConstType, dtype: DType) -> str:
    if math.isnan(x):
      ...
      val = f"{x}f" if dtypes.is_float(dtype) else f"{x}"
    return (self.render_cast([val] * dtype.count, dtype) if dtype.count > 1 or dtype not in [dtypes.float, dtypes.int,
                                                                                             dtypes.bool] else val)
  ...
  def render_kernel(self, function_name: str, kernel: List[str], bufs: List[Tuple[str, Tuple[DType, bool]]],
                    uops: UOpGraph, prefix=None) -> str:
    tmp = "const sampler_t smp = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;\n" if any(
      isinstance(dtype, ImageDType) for _, (dtype, _) in bufs) else ""  # noqa: E501
    buftypes = [(name, f"{'write_only' if mutable else 'read_only'} image2d_t" if dtype.name.startswith('image') else
    ("" if mutable else "const ") + self.buffer_prefix + self.render_dtype(
      dtype) + "*" + self.buffer_suffix if isinstance(dtype, PtrDType) else
  ...
  def render_store(self, buf_name: str, buf_dtype: DType, var_name: str, var_dtype: DType, idx: str,
                   local=False) -> str:
    if isinstance(buf_dtype, ImageDType):
  ...
  def render_local(self, name: str, dtype: DType, size: int):
    return self.smem_align + self.smem_prefix + f"{self.render_dtype(dtype)} {name}[{size}];"

  def render_dtype(self, var_dtype: DType) -> str:
    return self.type_map.get(var_dtype, var_dtype.name)
  ...
  def render(self, name: str, uops: UOpGraph) -> str:
    ...
    for u in uops:
      ...
      if uop is UOps.IF:
        ...
        rendered_store = self.render_store(r[vin[0]], vin[0].dtype, r[vin[2]], vin[2].dtype, strip_parens(r[vin[1]]),
                                           vin[0].uop is UOps.DEFINE_LOCAL)
        ...
        if uop is UOps.LOOP:
          ...
          if child_count[u] <= 1 and args is not BinaryOps.MAX and not getenv("EXPAND_SSA"):
            r[u] = val
          else:
            kk(f"{self.render_dtype(dtype)} {ssa('alu', u)} = {val};")
        elif uop is UOps.SPECIAL:
          kk(f"int {args[1]} = {self.code_for_workitem[args[1][0]](args[0])}; /* {args[2]} */")
          r[u] = args[1]
        elif uop is UOps.LOAD:
          val = self.render_load(dtype, r[vin[0]], vin[0].dtype, strip_parens(r[vin[1]]),
                                 vin[0].uop is UOps.DEFINE_LOCAL)
          ...
          if child_count[u] <= 1:
            ...
            kk(f"{self.render_dtype(dtype)} {ssa('cast', u)} = {val};")
          ...
          kk(f"{self.render_dtype(dtype)} {ssa('wmma', u)} = __{args[0]}({r[vin[0]]}, {r[vin[1]]}, {r[vin[2]]});")
        elif uop is UOps.DEFINE_ACC:
          kk(f"{self.render_dtype(dtype)} {ssa('acc', u)} = {self.render_const(args[0], dtype)};")
          ...
          r[u] = (r[vin[0]] if from_ssa else f"{(r[vin[0]])}") + (
            f"[{args}]" if vin[0].dtype.count > 4 else f".{'xyzw'[args]}")
        else:
...
class MetalRenderer(CStyleLanguage):
  device = "METAL"
  has_tensor_cores = platform.machine() == "arm64"
  shared_max = 32768
  ...
  code_for_op = {**CStyleLanguage().code_for_op,
                 BinaryOps.MAX: lambda a, b,
                                       dtype: f"(bfloat)max((float){a},(float){b})" if dtype == dtypes.bfloat16 else f"max({a},{b})",
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    prefix, wmma_args = ["#include <metal_stdlib>", "using namespace metal;"], set(
      [uop.arg for uop in uops if uop.uop is UOps.WMMA])
    for arg in wmma_args:
      prefix.append(f"""{arg[3].name}2 __{arg[0]}({arg[2].name}2 m, {arg[2].name}2 n, {arg[3].name}2 o) {{
...
extern "C" {\n""" + "".join([
  ...
  __attribute__((device)) {dt} __ocml_sin_f{n}({dt});\n""" for dt, n in
    [("float", 32), ("double", 64), ("_Float16", 16)]]) + \
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    vec_dts = [("float", "float", 2), ("float", "float", 4), ("float", "float", 8), ("signed int", "int", 4),
               ("signed int", "int", 2)]
...
""")
    ...
    for arg in set([uop.arg for uop in uops if uop.uop is UOps.WMMA]):
      if arg[3] == dtypes.float:
        prefix.append(f"#define __{arg[0]} __builtin_amdgcn_wmma_f32_16x16x16_f16_w32")
      else:
        prefix.append(f"static __attribute__((device)) half8 __{arg[0]}" + """(half16 a, half16 b, half8 c) {
