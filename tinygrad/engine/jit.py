...
import functools, itertools, collections, threading
...
def apply_graph_to_jit(jit_cache: List[ExecItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]) -> List[ExecItem]:
    ...
    for ji in jit_cache:
        ...
        graph_class = (ji_graph_dev.graph.func if isinstance(ji_graph_dev.graph,
                                                             functools.partial) else ji_graph_dev.graph) if ji_graph_dev else None
        can_be_graphed = ji_graph_dev and ji_graph_dev.graph
        can_share_graph = (ji_graph_dev == current_device or (
                    isinstance(graph_class, type) and issubclass(graph_class, MultiGraphRunner)) and
        ...
        if not can_extend_graph_batch and current_batch: flush_batch()
    ...
    def __init__(self, jit_cache: List[ExecItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]):
        ...
        for j, ji in enumerate(jit_cache):
            ...
            if isinstance(ji.prg, CompiledRunner):
                ...
                if (ji.prg.p.global_size and not all_int(ji.prg.p.global_size)) or (
                        ji.prg.p.local_size and not all_int(ji.prg.p.local_size)):
...
class TinyJit(Generic[ReturnType]):
    ...
    def add_buffer(self, b: Buffer) -> Buffer:
        ...
        if b._base is not None:
            self.buffer_replace[b] = ret = Buffer(b.device, b.size, b.dtype,
                                                  base=self.buffer_replace.get(b._base, b._base), offset=b.offset)
        else:
    ...
    def __call__(self, *args, **kwargs) -> ReturnType:
        input_tensors: List[Tuple[Union[int, str], Tensor]] = \
            [(cast(Union[int, str], k), v) for k, v in itertools.chain(enumerate(args), sorted(kwargs.items())) if
             v.__class__ is Tensor]
        if input_tensors: Tensor.realize(*[x[1] for x in input_tensors])
        ...
        var_vals: Dict[Variable, int] = merge_dicts([x[1] for x in expected_sts_var_dtype_device] + \
                                                    [dict(x.unbind() for x in itertools.chain(args, kwargs.values()) if
                                                          isinstance(x, Variable))])

        expected_names, expected_lbs = [x[0] for x in input_tensors], [(x[0], tuple(x[1].keys()), x[2], x[3]) for x in
                                                                       expected_sts_var_dtype_device]
        if self.cnt == 0:
            ...
            self.expected_lbs = expected_lbs
                ...
                for b in ji.bufs:
                    if b is not None and b._base is not None and b._base in input_rawbuffers:
                        ...
                        self.extra_view_inputs.append(
                            (input_rawbuffers.index(b.base), b.offset, b.device, b.size, b.dtype))

            assigned = _internal_memory_planner([cast(List[Buffer], x.bufs) for x in self.jit_cache],
                                                debug_prefix="JIT ")
            self.jit_cache = [
                ExecItem(ei.prg, [assigned.get(x, x).ensure_allocated() for x in ei.bufs if x is not None]) for ei in
                self.jit_cache]

            if JIT < 2: self.jit_cache = apply_graph_to_jit(self.jit_cache, input_rawbuffers, var_vals)

            self.input_replace = get_input_replace(self.jit_cache, input_rawbuffers)
            if DEBUG >= 1 and len(set(self.input_replace.values())) != len(input_rawbuffers): print(
                "WARNING: some input tensors not found")
        elif self.cnt >= 2:
            ...
            for idx, offset, device, size, dtype in self.extra_view_inputs:
                input_rawbuffers.append(
                    Buffer(device, size, dtype, base=input_rawbuffers[idx], offset=offset).ensure_allocated())
            ...
            def run_speculatively(ei, var_vals):
                ei.run(var_vals, jit=True)

            for ei in self.jit_cache:
                thread = threading.Thread(target=run_speculatively, args=(ei, var_vals))
                thread.start()
